...
#include <BLE2901.h>      // Descriptor para descripción de características
...
/// Contador que se incrementa y envía a la aplicación móvil
uint32_t value = 0;
...
class MyServerCallbacks : public BLEServerCallbacks {
  ...
  /**
   * Callback ejecutado cuando un cliente BLE se desconecta
...
class MyCallbacks : public BLECharacteristicCallbacks {
  ...
  void onWrite(BLECharacteristic *pCharacteristic) {
    ...
    if (rxValue.length() >= 5) {
      ...
      /// Si el dispositivo se apaga, resetear el contador a 0
      /// Esto asegura que cuando se vuelva a encender, comience desde cero
      if (!isOn) {
        value = 0;
        Serial.println("Dispositivo apagado - Contador reseteado a 0");
      }
...
void loop() {
  ...
  if (deviceConnected && isOn) {
    ...
    /// Convertir el contador a formato de 4 bytes (uint32_t)
    /// Los datos se envían en formato little-endian como espera Flutter
    pCharacteristic->setValue((uint8_t *)&value, 4);
    
    // ==================== ENVÍO DE NOTIFICACIÓN ====================
    /// Enviar notificación BLE al cliente conectado (Flutter)
    /// Esta notificación es recibida automáticamente por GetBle en Flutter
    pCharacteristic->notify();
    
    // ==================== INCREMENTO DEL CONTADOR ====================
    /// Incrementar el contador para la próxima notificación
    /// El contador se reinicia automáticamente al alcanzar el máximo uint32_t
    value++;
    
    // ==================== LOGGING DE DEPURACIÓN ====================
    /// Mostrar información del envío cada 10 notificaciones para no saturar el log
    if (value % 10 == 0) {
      Serial.print("Notificación enviada - Contador: ");
      Serial.print(value);
      Serial.print(", Intervalo: ");
      Serial.print(timeCheck);
      Serial.println(" ms");
    ...
    /// Esperar el tiempo configurado desde Flutter antes de la próxima notificación
    /// Este valor es dinámico y se actualiza con cada comando desde la app
    delay(timeCheck);
  ...
  if (deviceConnected && !oldDeviceConnected) {
    ...
    /// Resetear el contador para empezar desde cero con el nuevo cliente
    value = 0;
    Serial.println("Contador reiniciado para nueva sesión");
  }
}
